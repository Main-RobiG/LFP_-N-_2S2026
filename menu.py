...
# Ruta absoluta a la carpeta del proyecto (donde está este archivo),
# para que las rutas a "archivos/" funcionen sin importar desde qué
# carpeta se ejecute "python main.py".
CARPETA_PROYECTO = os.path.dirname(os.path.abspath(__file__))
CARPETA_ARCHIVOS = os.path.join(CARPETA_PROYECTO, "archivos")
...
def opcion_cargar_sudokus(tableros):
    # El enunciado exige que el archivo se llame sudokus.lfp (no tableros.lfp)
    nuevos = cargar_tableros(os.path.join(CARPETA_ARCHIVOS, "sudokus.lfp"))
    if nuevos:
        tableros.clear()
...
def opcion_cargar_jugadores(jugadores):
    nuevos = cargar_jugadores(os.path.join(CARPETA_ARCHIVOS, "jugadores.lfp"))
    if nuevos:
        jugadores.clear()
...
def opcion_cargar_intentos(intentos):
    nuevos = cargar_intentos(os.path.join(CARPETA_ARCHIVOS, "intentos.lfp"))
    if nuevos:
        intentos.clear()
